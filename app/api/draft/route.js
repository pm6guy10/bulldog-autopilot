...
async function buildSurgicalDoc(dossier, selectedArgs, tone) {
    ...
    if (selectedArgs.includes('bad_faith')) {
        doc.Sections.Children.push(new Paragraph({ text: "A. The Agency's Pattern of Constructive Denials Demonstrates Bad Faith", heading: HeadingLevel.HEADING_2 }));
        doc.Sections.Children.push(new Paragraph({ text: buildBadFaithArgument(dossier) }));
    }
    if (selectedArgs.includes('privilege_waiver')) {
        doc.Sections.Children.push(new Paragraph({ text: "B. The Agency Has Waived Exemptions By Failing to Provide Compliant Privilege Logs", heading: HeadingLevel.HEADING_2 }));
        doc.Sections.Children.push(new Paragraph({ text: buildPrivilegeWaiverArgument(dossier) }));
    }
    if (selectedArgs.includes('in_camera_review')) {
        doc.Sections.Children.push(new Paragraph({ text: "C. The Court Must Conduct an In Camera Review", heading: HeadingLevel.HEADING_2 }));
        doc.Sections.Children.push(new Paragraph({ text: buildInCameraReviewArgument(dossier) }));
    }
    doc.Sections.Children.push(new Paragraph({ text: "III. CONCLUSION", heading: HeadingLevel.HEADING_1 }));
    doc.Sections.Children.push(new Paragraph({ text: "For the foregoing reasons, the Court should order the immediate release of all non-exempt records and award statutory penalties and attorneys' fees." }));
    if (tone === 'aggressive') {
        doc.Sections.Children.push(new Paragraph({ text: "Furthermore, the Court must issue significant monetary sanctions against the Agency for its bad faith conduct." }));
    }
    doc.Sections.Children.push(new Paragraph({ text: "\n\nDated this ___ day of September, 2025." }));
    doc.Sections.Children.push(new Paragraph({ text: "\n\n\t________________________________" }));
    doc.Sections.Children.push(new Paragraph({ text: "\t[YOUR NAME], WSBA #[NUMBER]" }));
    doc.Sections.Children.push(new Paragraph({ text: "\tAttorney for Plaintiff" }));
