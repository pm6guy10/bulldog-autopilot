  ...
  <div className="max-w-4xl mx-auto space-y-8">
    ...
    <main className="space-y-4">
      ...
      <div className="space-y-3">
        {matters.map((matter) => (
          <motion.div key={matter.id}
            onClick={() => onSelect(matter)}
            className="bg-[var(--panel)] p-4 rounded-lg border border-[var(--border)]
              cursor-pointer transition-all hover:border-[var(--accent)] hover:bg-slate-800">
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <main className="grid md:grid-cols-2 gap-6">
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg text-center md:col-span-2">
          <h3 className="font-semibold text-[var(--text-secondary)]">Total Violations Logged</h3>
          <p className="text-6xl font-bold text-white mt-2">{summary.total ?? "-"}</p>
        </div>
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg">
          <h3 className="font-semibold text-[var(--text-secondary)] mb-4">Live Case Metrics</h3>
          <div className="space-y-3">
            <div className="flex justify-between"><span>High-Risk Violations:</span>
              <span className="font-bold">{summary.factors?.high_risk_count ?? "-"}</span></div>
            <div className="flex justify-between"><span>Constructive Denials:</span>
              <span className="font-bold">{summary.factors?.denial_count ?? "-"}</span></div>
            <div className="flex justify-between"><span>Privilege Log Failures:</span>
              <span className="font-bold">{summary.factors?.privilege_log_failures ?? "-"}</span></div>
            <div className="flex justify-between"><span>Average Delay (Days):</span>
              <span className="font-bold">{summary.factors?.avg_delay ?? "-"}</span></div>
          </div>
        </div>
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg">
          <h3 className="font-semibold text-[var(--text-secondary)] mb-4">Yousoufian Score</h3>
        ...
        <div className="md:col-span-2 flex justify-center mt-4">
          <button onClick={onDraft}
            className="btn-primary font-bold py-3 px-6 rounded-lg w-full max-w-sm flex items-center justify-center gap-2"
            disabled={isDrafting}>
            {isDrafting ? <><Loader2 size={18} className="animate-spin" /> Drafting...</>
...
export default function Page() {
  ...
  const handleDraftMotion = async () => {
    ...
    } catch (err) { alert(`Drafting Failed: ${err.message}`); }
      ...
      <AnimatePresence mode="wait">
        {!selectedMatter ? (
          ...
          <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <MatterDashboard matter={selectedMatter} summary={summary} onBack={() => setSelectedMatter(null)}
              onDraft={handleDraftMotion} isDrafting={isDrafting} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
