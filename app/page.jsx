...
import { ChevronRight, Loader2, BrainCircuit, FileText } from "lucide-react";
...
const MatterSelectionScreen = ({ matters, onSelect, connected }) => (
  <div className="max-w-4xl mx-auto space-y-8">
    <header className="space-y-4">
      <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl sm:text-5xl font-bold glow-text">
      ...
      <main className="grid md:grid-cols-2 gap-6">
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg text-center md:col-span-2">
          ...
          <p className="text-6xl font-bold text-white mt-2">
            {summary.total === null ? <Loader2 className="animate-spin inline" /> : summary.total}
          </p>
        </div>
        ...
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg">
          ...
          <div className="space-y-3">
            <div className="flex justify-between"><span>High-Risk Violations:</span> <span className="font-bold">{summary.factors?.high_risk_count ?? "-"}</span></div>
            <div className="flex justify-between"><span>Constructive Denials:</span> <span className="font-bold">{summary.factors?.denial_count ?? "-"}</span></div>
            <div className="flex justify-between"><span>Privilege Log Failures:</span> <span className="font-bold">{summary.factors?.privilege_log_failures ?? "-"}</span></div>
            <div className="flex justify-between"><span>Average Delay (Days):</span> <span className="font-bold">{summary.factors?.avg_delay ?? "-"}</span></div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 flex justify-center mt-4">
          <button
            onClick={onDraft}
            disabled={isDrafting}
            className="btn-primary py-3 px-6 w-full max-w-sm flex items-center justify-center gap-2"
          >
            {isDrafting ? (
              <><Loader2 size={18} className="animate-spin" /> Drafting...</>
            ) : (
              <><FileText size={18} /> Draft Motion</>
            )}
          </button>
        </div>
      </main>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    setSummary({ total: null, factors: null }); // Reset summary to show loading state
  ...
  const handleDraftMotion = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Drafting request failed");
      }
      const { draft } = await response.json();
      // Simple text download for now, as pleading paper was causing issues
