...
function StatusPill({ connected }) {
  return (
    <div className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium ${ connected ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400" }`}>
  ...
  <div className="max-w-4xl mx-auto space-y-8">
    ...
    <main className="space-y-4">
      ...
      <div className="space-y-3">
        {(matters || []).map((matter) => (
          <motion.div key={matter.id} onClick={() => onSelect(matter)} className="bg-[var(--panel)] p-4 rounded-lg border border-[var(--border)] cursor-pointer transition-all hover:border-[var(--accent)] hover:bg-slate-800">
...
const MatterDashboard = ({ matter, summary, onBack, onDraft, isDrafting }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <header><button onClick={onBack} className="text-sm text-[var(--text-secondary)] hover:text-white transition-colors mb-4">&larr; Back to All Matters</button><h1 className="text-4xl sm:text-5xl font-bold">{matter.name}</h1></header>
      <main className="grid md:grid-cols-2 gap-6">
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg text-center md:col-span-2"><h3 className="font-semibold text-[var(--text-secondary)]">Total Violations Logged</h3><p className="text-6xl font-bold text-white mt-2">{summary.total ?? <Loader2 className="animate-spin inline" />}</p></div>
        ...
        <div className="bg-[var(--panel)] p-6 border border-[var(--border)] rounded-lg"><h3 className="font-semibold text-[var(--text-secondary)] mb-4">Yousoufian Score</h3><ResponsiveContainer width="100%" height={200}><RadarChart cx="50%" cy="50%" outerRadius="80%" data={factorData}><PolarGrid stroke="var(--border)" /><PolarAngleAxis dataKey="subject" tick={{ fill: "var(--text-secondary)", fontSize: 12 }} /><Radar name="Score" dataKey="A" stroke="var(--accent)" fill="var(--accent)" fillOpacity={0.6} /></RadarChart></ResponsiveContainer></div>
        <div className="md:col-span-2 flex justify-center mt-4"><button onClick={onDraft} className="btn-primary font-bold py-3 px-6 rounded-lg w-full max-w-sm flex items-center justify-center gap-2" disabled={isDrafting}>{isDrafting ? <><Loader2 size={18} className="animate-spin" /> Drafting...</> : <><BrainCircuit size={18} /> Ask AI Partner to Draft Motion</>}</button></div>
      </main>
    </div>
  );
};
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function fetchSummary() {
      ...
      } catch (err) { setSummary({ total: "Error", factors: null }); }
    }
    fetchSummary();
  }, [selectedMatter]);
  ...
  const handleDraftMotion = async () => {
    ...
    setIsDrafting(true);
    try {
      const response = await fetch("/api/draft/motion", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ matter_id: selectedMatter.id }), });
      if (!response.ok) throw new Error((await response.json()).error || "Drafting failed");
      const { draft } = await response.json();
      const blob = new Blob([draft], { type: 'text/plain;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `${selectedMatter.name}_Motion_Draft.txt`;
      a.click(); URL.revokeObjectURL(url);
    } catch (error) { alert(`Drafting Failed: ${error.message}`); } 
